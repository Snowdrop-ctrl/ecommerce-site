...
export class OrdersService {
  ...
  async create(userId: number, body: OrderPlaceDto) {
    ...
    const calculateTotalAmount = products.reduce(
    ...
    await this.orderModel.transaction(async (trx) => {
      ...
      await this.orderItemModel
        .query(trx)
        .insertGraph(createOrderItemDataToInsert);
  ...
  async getAll(query: OrdersListDto) {
    ...
    const orderListQueryBuilder = this.orderModel
      .query()
      .limit(limit)
      .offset((page - 1) * limit);

    const totalOrdes = (await orderListQueryBuilder
      .clone()
      .count('id')
      .first()) as unknown as { count: string };

    return {
      items: await orderListQueryBuilder,
      total: +totalOrdes?.count || 0,
    };
  }
